...
object Sessionize {
  ...
  val iterator = GzFileIterator(new java.io.File("2015_07_22_mktplace_shop_web_log_sample.log.gz"), "UTF-8")
  ...
  def parseLine(line: String) = {
    ...
    if(matcher.find()) {
      val logLine = LogLine.newBuilder()
        .setIp(matcher.group(3))
        .setTimestamp(new SimpleDateFormat("dd/MMM/yyyy:hh:mm:ss").parse(matcher.group(1)).getTime)
        .setUrl(matcher.group(12))
        // .setReferrer(matcher.group(6))
        .setUseragent(matcher.group(13))
